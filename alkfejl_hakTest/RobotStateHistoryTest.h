 ...
 * @note Ide és a alkfejl_hakLib-be kerülnek a unit testekhez szükséges sorok.
 *
 */
...
public:
    ...
    /* historyHasChanged() függvény tesztelése - RobotStateHistory.cpp-ben található */
    int historyHasChangedTest(const RobotState& state);

    /* clearScreen() függvény tesztelése - RobotStateHistory.cpp-ben található */
    int clearScreenTest(const RobotState& state);

    /* visualize() függvény tesztelése - RobotStateHistory.cpp-ben található */
